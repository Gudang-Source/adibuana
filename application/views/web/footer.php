<footer id="footer" class="page-footer footer-preset-5">
        <div class="footer-body bgc-gray-darker">
          <div class="footer-logo">
      <img src="<?php echo base_url(); ?>assets/images/logo/logo_utama.png" />
  </div>
          <div class="container container-home">
            <div class="row">
              <div class="col-lg-3 col-sm-6 col-xs-12 __block-wrapper-1 col-md-offset-1">
                <div class="footer-widget-about footer-widget-contact">
                    <!--<a href="<?php echo base_url() ?>"><img src="<?php echo base_url() ?>assets/images/logo.png" alt=""></a>-->
                    
                    <div class="__content font-heading fz-6-ss">
                        <h3>Kampus 1</h3>
                        <div class="__address"><i class="icon icon-location"></i><span>Jl. Ngagel Dadi III-B/37, Fax. (031) 5042804, Surabaya, 60245</span></div>
                        <div class="__phone"><i class="icon icon-phone-1"></i><span>(123) 45678910</span></div>
                        <div class="__email"><i class="icon icon-paper-plane"></i><span>info@osthemes.com</span></div>
                    </div>
                </div>
              </div>
              <div class="col-lg-3 col-sm-6 col-xs-12 __block-wrapper-2 col-md-offset-1">
                <div class="footer-widget-gallery footer-widget-contact">
                  <!--<h4>&nbsp</h4>-->
                  <div class="__content font-heading fz-6-ss">
                        <h3>Kampus 2</h3>
                        <div class="__address"><i class="icon icon-location"></i><span>Jl. Ngagel Dadi III-B/37, Fax. (031) 5042804, Surabaya, 60245</span></div>
                        <div class="__phone"><i class="icon icon-phone-1"></i><span>(123) 45678910</span></div>
                        <div class="__email"><i class="icon icon-paper-plane"></i><span>info@osthemes.com</span></div>
                    </div>
                </div>
              </div>
              
              
            </div>
          </div>
        </div>

        <div class="footer-foot-1 bgc-gray-base">
          <div class="container text-center">
            <p class="font-heading fz-6-s">© 2017. Universitas Adi Buana Surabaya. All Rights Reserved</p>
          </div>
        </div>

</footer>